...
namespace How_to_create_HMI_Control_Real_Time
{
    public partial class FormMain : Form
    {
        ...
        const int ADD_Z_MOVE = 33;
        const int ADD_XWP = 34;
        const int ADD_YWP = 35;
        const int ADD_ZWP = 36;

        const int ADD_XDSP = 37;
        const int ADD_YDSP = 38;
        const int ADD_ZDSP = 39;
        ...
        private void CHAY_HANHTRINH()
        {
            ...
            if (Type_spray[hanhtrinh_so] == Rectangular_Array && REQ_DISSPRAY == 0)
            {
                ...
                {
                    MoveXY(X2_spray[hanhtrinh_so] + deX * (_cell_number - 1), Y1_spray[hanhtrinh_so] + deY * (_cell_number - 1));
                    Draw_hanhtrinh(_coodinateX, _coodinateY, X2_spray[hanhtrinh_so], Y1_spray[hanhtrinh_so]);
                    _runX = _runY = 1;
                    _step++;
                }
                //-================== lap lai  X TO Y  ==============================
                ...
                else if(_step == 5)// && _runX == 0 && _runY == 0)
                {
                        if (_runX == 0 && _runY == 0 && _tangbacXY == 1)
                        {
                            ...
                            {
                                MoveXY(X2_spray[hanhtrinh_so] + deX * (_cell_number - 1), Y1_spray[hanhtrinh_so] + deY * (_cell_number - 1)  + Dir_step * step_num_array * _stepY);  // hoac X1
                                Draw_hanhtrinh(_coodinateX, _coodinateY, X2_spray[hanhtrinh_so], Y1_spray[hanhtrinh_so] + Dir_step * step_num_array * _stepY);
                            }
                            else
                            {
                                MoveXY(X1_spray[hanhtrinh_so] + deX * (_cell_number - 1), Y1_spray[hanhtrinh_so] + deY * (_cell_number - 1)  + Dir_step * step_num_array * _stepY);
                                Draw_hanhtrinh(_coodinateX, _coodinateY, X1_spray[hanhtrinh_so], Y1_spray[hanhtrinh_so] + Dir_step * step_num_array * _stepY);
                            }
                            _runX = _runY = 1;                          
                            _tangbacXY = 2;
                        }
                        if(_runX == 0 && _runY == 0 && _tangbacXY == 2)
                        {
                            ...
                            {
                                MoveXY(X2_spray[hanhtrinh_so] + deX * (_cell_number - 1), Y1_spray[hanhtrinh_so] + deY * (_cell_number - 1)  + Dir_step * step_num_array * _stepY);
                                Draw_hanhtrinh(_coodinateX, _coodinateY, X2_spray[hanhtrinh_so], Y1_spray[hanhtrinh_so] + Dir_step * step_num_array * _stepY);
                            }
                            _runX = _runY = 1;
                            _tangbacXY = 1;
      ...
      void IC_but_DoubleClick(object sender, EventArgs e)
        ...
        private void DT_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            if(TYPE_HANHTRINH == POINT)
            {
                if(ID_PL.Text != "" && X1.Text != "" && Y1.Text != "" && Z1.Text != "")
                {
                    ID_HANHTRINH++;
                    ID_PL.Text = ID_HANHTRINH.ToString();
                    table.Rows.Add(ID_PL.Text,Type_PL.Text, X1.Text, Y1.Text, Z1.Text,"0","0","0","0",CB.CheckState);
                    DT.DataSource = table;
                }
                else
                { 
            ...
            if(TYPE_HANHTRINH == LINE)
            {
                if (ID_PL.Text != "" && X1.Text != "" && Y1.Text != "" && Z1.Text != "" && X2.Text != "" && Y2.Text != "" && Z2.Text != "")
                {
                    ID_HANHTRINH++;
                    ID_PL.Text = ID_HANHTRINH.ToString();
                    table.Rows.Add(ID_PL.Text, Type_PL.Text, X1.Text, Y1.Text, Z1.Text, X2.Text, Y2.Text, Z2.Text, "0", Speed_manual.Text, CB.CheckState);
                    DT.DataSource = table;
                }
                else
                {
            ...
            if(TYPE_HANHTRINH == Rectangular_Array)
            {
                if (ID_PL.Text != "" && X1.Text != "" && Y1.Text != "" && Z1.Text != "" && X2.Text != "" && Y2.Text != "" && Z2.Text != "" && Number_step.Text != "")
                {
                    ID_HANHTRINH++;
                    ID_PL.Text = ID_HANHTRINH.ToString();
                    table.Rows.Add(ID_PL.Text, Type_PL.Text, X1.Text, Y1.Text, Z1.Text, X2.Text, Y2.Text, Z2.Text, Number_step.Text, Speed_manual.Text, CB.CheckState);
                    DT.DataSource = table;
                }
                else
                {
        ...
        private void DT_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            //if (DT.Rows[0].Cells[1].Value != null)
            //    // MessageBox.Show(DT.Rows[0].Cells[2].Value.ToString());
            //    MessageBox.Show("Bạn muốn thay đổi dữ liệu di chuyển?");
            //else
            //    MessageBox.Show("NULL");
        ...
        private void Save_data_Click(object sender, EventArgs e)
        {
            ...
            for(int i =0;i < (DT.RowCount-1);i++)
            {
                 if (String.Compare(DT.Rows[i].Cells[1].Value.ToString(), "Point") == 0)  //=================>> TOADO_1 = TOA_DO_2
                {
                    ...
                    //--------------------- X2 ------------------------
                    val = 0;// ushort.Parse(DT.Rows[i].Cells[5].Value.ToString());
                    val16 = BitConverter.GetBytes(val);
                    data[Add_START + i * SIZE_DATA + 7] = val16[0];
                    data[Add_START + i * SIZE_DATA + 8] = val16[1];
                    //--------------------- Y2 ------------------------
                    val = 0;// ushort.Parse(DT.Rows[i].Cells[6].Value.ToString());
                    val16 = BitConverter.GetBytes(val);
                    data[Add_START + i * SIZE_DATA + 9] = val16[0];
                    data[Add_START + i * SIZE_DATA + 10] = val16[1];

                    //--------------------- Z2 ------------------------
                    val = 0;// ushort.Parse(DT.Rows[i].Cells[7].Value.ToString());
                    val16 = BitConverter.GetBytes(val);
            ...
            File.WriteAllBytes("Toadochamkeo.tula", data);
        ...
        private void Call_data_Click(object sender, EventArgs e)
        {
            ...
            ushort val = 0;
            //=====================================================================================
            ushort X1_T, Y1_T, Z1_T, X2_T, Y2_T, Z2_T, STEP_T, SPEED_T, LIFT_UP_T;
            int Type_T;
            //=============================  DATA IC_STACK ========================================//
            byte[] Data_toado = File.ReadAllBytes("Toadochamkeo.tula");
            ...
            for (int i = 0; i < sohanhtrinh; i++)
            {
                ...
                if (Type_T == POINT)
                    table.Rows.Add((i + 1), "Point", X1_T, Y1_T, Z1_T,"0", "0", "0", "0", CB.CheckState);
                if (Type_T == LINE)
                    table.Rows.Add((i + 1), "Line", X1_T.ToString(), Y1_T.ToString(), Z1_T.ToString(), X2_T.ToString(), Y2_T.ToString(), Z2_T.ToString(), "0", SPEED_T.ToString(), CB.CheckState);                
                if(Type_T == Rectangular_Array)
                {
                    table.Rows.Add((i + 1), "RECTANG", X1_T.ToString(), Y1_T.ToString(), Z1_T.ToString(), X2_T.ToString(), Y2_T.ToString(), Z2_T.ToString(), STEP_T.ToString(), SPEED_T.ToString(), CB.CheckState);
                }

            }
            DT.DataSource = table;
            //============================================  Z AUTO MOVE ============================================
            byte[] data_auto_move = File.ReadAllBytes("Data_automove.tula");
            val16[0] = data_auto_move[1];
            val16[1] = data_auto_move[2];
            Zmove = BitConverter.ToUInt16(val16, 0);
    
            Z_auto_move.Text = Zmove.ToString();
            //=========================================== xyx WAITING ==============================================
            val16[0] = data_auto_move[3];
            val16[1] = data_auto_move[4];
            PW_X = BitConverter.ToUInt16(val16, 0);
            Posw_x.Text = PW_X.ToString();

            val16[0] = data_auto_move[5];
            val16[1] = data_auto_move[6];
            PW_Y = BitConverter.ToUInt16(val16, 0);
            Posw_y.Text = PW_Y.ToString();

            val16[0] = data_auto_move[7];
            val16[1] = data_auto_move[8];
            PW_Z = BitConverter.ToUInt16(val16, 0);
            Posw_z.Text = PW_Z.ToString();

            //==================================== limit z ==============================================
            val16[0] = data_auto_move[9];
            val16[1] = data_auto_move[10];
            Zlimit = BitConverter.ToUInt16(val16, 0);
            Z_limit.Text = Zlimit.ToString();
            LIMIT_Z.Text = Zlimit.ToString();
            //================================== Disspray ================================
            val16[0] = data_auto_move[11];
            val16[1] = data_auto_move[12];
            Dsp_X = BitConverter.ToUInt16(val16, 0);
            DSP_X.Text = Dsp_X.ToString();

            val16[0] = data_auto_move[13];
            val16[1] = data_auto_move[14];
            Dsp_Y = BitConverter.ToUInt16(val16, 0);
            DSP_Y.Text = Dsp_Y.ToString();

            val16[0] = data_auto_move[15];
            val16[1] = data_auto_move[16];
            Dsp_Z = BitConverter.ToUInt16(val16, 0);
            DSP_Z.Text = Dsp_Z.ToString();
            //======================== time drop =========================================
            val16[0] = data_auto_move[17];
            val16[1] = data_auto_move[18];
            Time_Drop = BitConverter.ToUInt16(val16, 0);
            val_timedrop.Text = Time_Drop.ToString();
            //======================= OF1 ============================================
            val16[0] = data_auto_move[19];
            val16[1] = data_auto_move[20];
            OF1x = BitConverter.ToUInt16(val16, 0);
            OF1_X.Text = OF1x.ToString();

            val16[0] = data_auto_move[21];
            val16[1] = data_auto_move[22];
            OF1y = BitConverter.ToUInt16(val16, 0);
            OF1_Y.Text = OF1y.ToString();
            //====================== OFn =============================================
            val16[0] = data_auto_move[23];
            val16[1] = data_auto_move[24];
            OFnx = BitConverter.ToUInt16(val16, 0);
            OFn_X.Text = OFnx.ToString();

            val16[0] = data_auto_move[25];
            val16[1] = data_auto_move[26];
            OFny = BitConverter.ToUInt16(val16, 0);
            OFn_Y.Text = OFny.ToString();
            //===================== number copy========================================
            val16[0] = data_auto_move[27];
            val16[1] = data_auto_move[28];
        ...
        private void Select_Speed_Click_1(object sender, EventArgs e)
        {

        }

        //private void PUMP_MouseDown(object sender, MouseEventArgs e)
        //{
        ...
        private void Save_Zmove_Click(object sender, EventArgs e)
        {
            ...
            data[28] = val16[1];
            //=========================================================
            File.WriteAllBytes("Data_automove.tula", data);
            MessageBox.Show("Save complete!");
